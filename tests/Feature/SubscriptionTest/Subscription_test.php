...
class Subscription_test extends FeatureTestCase
{
    ...
    {
        parent::setUp();
        $this->truncateTables([PartnerSubscriptionPackage::class, Partner::class, Resource::class, PartnerResource::class, Tag::class]);

        $this->logIn();
        $this->freeSubscription = factory(PartnerSubscriptionPackage::class)->create();
        $this->basicSubscription = factory(PartnerSubscriptionPackage::class)->create(['name' => "Basic"]);
        Tag::create(['name' => "Subscription fee", 'taggable_type' => 'App\\Models\\PartnerTransaction',]);
    ...
    public function testSubscriptionRules()
    {
        ...
        $respose = $this->get("v2/partners/38420/subscriptions/all-packages?remember_token=$resource_remembar_token", ["version-code" => "21121"]);
