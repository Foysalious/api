...
class UserProfileUpdateV1Test extends FeatureTestCase
{
    ...
    public function testUserProfileUpdateAPIV1ByUpdatingNameGenderBirthdayAddress()
    {
        ...
        $response = $this->put(
            "/v1/customers/".$this->customer->id."/edit?remember_token=".$this->customer->remember_token,
            [
                'field' => 'name',
                'value' => 'John Doe',
    ...
    public function testUserProfileUpdateAPIV1ByGivingCharacterInputInBirthday()
    {
        ...
        $this->assertEquals(400, $data["code"]);
        $this->assertEquals(
            "The value is not a valid date.The value does not match the format Y-m-d.The value must be a date before ".$today.".",
            $data["message"]
        );
    }
