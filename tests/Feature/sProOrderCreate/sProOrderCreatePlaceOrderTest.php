...
class sProOrderCreatePlaceOrderTest extends FeatureTestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->master_category_location_id = CategoryLocation::factory()->create([
        ...
        $this->secondary_category_location_id = CategoryLocation::factory()->create([
        ...
        $this->location_service = LocationService::factory()->create([
        ...
        DB::insert(
            'insert into partner_service(partner_id, service_id, is_verified, is_published) values (?, ?, ?, ?)',
            [$this->partner->id, $this->service->id, 1, 1]
        );

        DB::insert(
            'insert into partner_wallet_settings(partner_id, min_withdraw_amount, max_withdraw_amount, security_money, security_money_received, min_wallet_threshold) values (?, ?, ?, ?, ?, ?)',
            [$this->partner->id, 1000, 10000, 500, 1, 1000]
        );

        DB::insert(
            'insert into partner_working_hours(partner_id, day, start_time, end_time) values (?, ?, ?, ?)',
            [$this->partner->id, 'Friday', '09:00:00', '18:00:00']
        );
