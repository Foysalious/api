...
class Bkash extends PaymentMethod
{
    ...
    public function validate(Payment $payment)
    {
        ...
        if ($execute_response->hasSuccess()) {
            ...
            try {
                (new Registrar())->setAmount($this->payable->amount)->setDetails(json_encode($success->details))->setTime(isset($success->details->createTime) ? Carbon::parse($success->details->createTime)->format('Y-m-d H:s:i') : null)->setIsValidated(1)->register($payment->payable->user, 'bkash', $success->id, $this->merchantNumber);
