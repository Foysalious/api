...
class TopUp
{
    ...
    public function recharge($mobile_number, $amount, $type)
    {
        ...
        if ($response->hasSuccess()) {
            ...
            DB::transaction(function () use ($response, $mobile_number, $amount) {
                ...
                $amount_after_commission = $amount - $this->agent->calculateCommission($amount, $this->vendor);
    ...
    private function refund(TopUpOrder $topUpOrder)
    {
        ...
        $amount_after_commission = round($amount - $agent->calculateCommission($amount, $this->vendor), 2);
