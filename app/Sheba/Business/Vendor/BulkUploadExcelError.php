...
class BulkUploadExcelError
{
    ...
    {
        if (!$this->excel) $this->excel = Excel::selectSheets(BulkUploadExcel::SHEET)->load($this->file);
        return $this->excel;
    }
    ...
    {
        if ($message) {
            $this->getExcel()->getActiveSheet()->setCellValue(BulkUploadExcel::MESSAGE_COLUMN . $this->row, $message);
            $this->excel->save();
        }
    }
