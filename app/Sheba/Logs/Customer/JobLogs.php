...
class JobLogs
{
    ...
    public function getOrderMessage()
    {
        ...
        if (in_array($job_status, [constants('JOB_STATUSES')['Pending'], constants('JOB_STATUSES')['Not_Responded']])) {
            ...
            $scheduled_date_time = Carbon::parse($this->job->schedule_date . ' ' . $this->job->preferred_time_start);

            if (($scheduled_date_time->isToday() && $scheduled_date_time->diffInMinutes(Carbon::now()) >= 30)) {
                return array(
                    'status' => 'message',
