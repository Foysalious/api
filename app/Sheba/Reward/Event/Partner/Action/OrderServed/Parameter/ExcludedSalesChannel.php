...
class ExcludedSalesChannel extends ActionEventParameter
{
    /**
    ...
    public function validate()
    {
        ...
        if (empty($this->value))
            throw new ParameterTypeMismatchException("Excluded Sales Channels can't be empty");

        $sales_channels = getSalesChannels();
        foreach ($this->value as $value) {
            if(!in_array($value, $sales_channels))
                throw new ParameterTypeMismatchException("Excluded Sales Channel can't be " . $value);
        }
    }
