...
class MovieTicket
{
    ...
    {
        $movie_ticket_order = $this->placeMovieTicketOrder($this->movieTicketRequest->getName(), $this->movieTicketRequest->getEmail(),
            $this->movieTicketRequest->getMobile(), $this->movieTicketRequest->getAmount());
...
//    }
    ...
    private function placeMovieTicketOrder($name, $email, $mobile_number, $amount)
    {
        ...
        $movie_ticket_order->reservation_details = json_encode([
            'trx_id' => $this->movieTicketRequest->getTrxId(),
            'dtmsid' => $this->movieTicketRequest->getDtmsId(),
            'lid' => $this->movieTicketRequest->getTicketId(),
            'image_url' => $this->movieTicketRequest->getImageUrl(),
            'original_price' => $amount - (($amount * $this->model->sheba_commission) /(100 + $this->model->sheba_commission))
        ]);
