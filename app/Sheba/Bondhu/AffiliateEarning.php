...
class AffiliateEarning implements PartnerAffiliationEarning
{
    ...
    private function creditWalletForPartnerAffiliation(PartnerAffiliation $affiliation, $reward, Affiliate $affiliate, $log, $is_gifted = 0)
    {
        ...
        (new WalletTransactionHandler())->setModel($affiliate)->setSource(TransactionSources::SHEBA_WALLET)->setType('credit')->setAmount($reward)->setLog($log)->dispatch(['is_gifted' => $is_gifted, 'affiliation_type' => get_class($affiliation), 'affiliation_id' => $affiliation->id]);
    }
    ...
    public function affiliation(Affiliate $affiliate, $amount, $affiliation_id, $order_code)
    {
        ...
        $log = "Earned $amount tk for reference: $affiliation_id and order: $order_code";
        (new WalletTransactionHandler())->setModel($affiliate)->setSource(TransactionSources::SHEBA_WALLET)->setType('credit')->setAmount($amount)->setLog($log)->dispatch(['affiliation_type' => "App\\Models\\Affiliation",
            'affiliation_id' => $affiliation_id]);
    }
    ...
    public function affiliationAmbassadorEarning(Affiliate $ambassador, Affiliate $affiliate, $amount, $affiliation_id, $order_code)
    {
        ...
        $log = "$affiliate_identity gifted $amount tk for reference: $affiliation_id and order: $order_code";
        (new WalletTransactionHandler())->setModel($affiliate)->setSource(TransactionSources::SHEBA_WALLET)->setType('credit')->setAmount($amount)->setLog($log)->dispatch(['affiliation_type' => "App\\Models\\Affiliation", 'is_gifted' => 1,
            'affiliation_id' => $affiliation_id]);
    }
