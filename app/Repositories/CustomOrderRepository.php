...
class CustomOrderRepository
{
    ...
    public function save($customer, $service, $request)
    {
        ...
        $custom_order->schedule_date = $request->has('schedule_date') ? $request->schedule_date : '';
        $custom_order->preferred_time = $request->has('preferred_time') ? $request->preferred_time : '';
        ...
        $custom_order->crm_id = $request->has('crm_id') ? $request->crm_id : null; //REMOVE
