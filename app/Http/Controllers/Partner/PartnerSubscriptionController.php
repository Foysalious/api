            ...
            if ($upgradeRequest = $this->createSubscriptionRequest($requestedPackage)) {
                try {
                    ...
                    if (!$request->partner->hasCreditForSubscription($requestedPackage, $request->billing_type)) {
                        ...
                        return api_response($request, null, 420, ['message' => 'আপনার একাউন্টে যথেষ্ট ব্যলেন্স নেই।।', 'remaining_balance' => round($request->partner->totalCreditForSubscription,2),'price'=>$request->partner->totalPriceRequiredForSubscription]);
                    }
