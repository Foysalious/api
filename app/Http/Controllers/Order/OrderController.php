...
//            'remember_token' => 'required|string',
    ...
    private function setModifierFromRequest(Request $request)
    {
        ...
        if ($request->filled('created_by')) $this->setModifier(User::find((int)$request->created_by));
    ...
    private function isFraudOrder(Request $request): bool
    {
        ...
        if ($request->filled('address') && $request->address == "Abdul Bariq Bhuaian Sharak, Korail, Dhaka")
            return true;
        if ($request->filled('address_id') && in_array($request->address_id, $fraudulent_address_id))
            return true;
