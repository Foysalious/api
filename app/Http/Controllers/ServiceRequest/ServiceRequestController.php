...
class ServiceRequestController extends Controller
{
    ...
    public function store(Request $request, ProfileRepositoryInterface $profile_repository, CustomerCreator $customer_creator, InfoCallCreator $creator)
    {
        ...
        if (!$customer) $customer = $customer_creator->setMobile($request->mobile)->setName($request->name)->create();
        $info_call = $creator->setServiceName($request->service_name)->setEstimatedBudget($request->estimated_budget)->setName($request->name)
            ->setMobile($request->mobile)->setCustomer($customer)->setLocationId($request->location_id)->create();
