...
class PosCustomerController extends Controller
{
    ...
    public function storePosCustomer(Request $request)
    {
        ...
        $customer = $this->posCustomerService->setPartner($partner)->setNote($request->note)->setName($request->name)
            ->setBnName($request->bnName)->setMobile($request->mobile)->setEmail($request->email)
            ->setAddress($request->address)->setSupplier($request->is_supplier)->setGender($request->gender)
            ->setBloodGroup($request->blood_group)->setDob($request->dob)->setproPic($image)
            ->setCreatedFrom($request->created_from)->storePosCustomer();
    ...
    public function storeSupplier(Request $request): JsonResponse
    {
        ...
        $supplier = $this->posCustomerService->setPartner($partner)->setNote($request->note)->setName($request->name)
            ->setBnName($request->bnName)->setMobile($request->mobile)->setEmail($request->email)
            ->setAddress($request->address)->setGender($request->gender)->setBloodGroup($request->blood_group)
            ->setDob($request->dob)->setproPic($image)->setCreatedFrom($request->created_from)->storeSupplier();
    ...
    {
        $partner = $request->auth_user->getPartner();
        $contacts = $this->posCustomerService->setPartner($partner)->getContactList($request->type);
