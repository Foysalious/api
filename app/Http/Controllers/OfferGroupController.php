        ...
        try {
            ...
            if ($location) {
                $offer_group = OfferGroup::with(['offers' => function ($q) use ($location) {
                    return $q->active()->valid_flsh()->flash()->select('id', 'target_type', 'target_id', 'start_date', 'end_date')
                ...
                $offer_group = OfferGroup::with(['offers' => function ($q) {
                    $q->active()->valid_flsh()->flash()->select('id', 'target_type', 'target_id', 'start_date', 'end_date');
                }])->where('id', $offer_group)->select('id', 'name', 'app_thumb')->first();
            }
