...
class ApprovalSettingTransformer extends TransformerAbstract
{
    /**
    ...
    {
        $this->department = $department_repo;
        $this->businessMember = $business_member_repo;
        $this->profile = $profile_repo;
     ...
     * @return mixed
     */
    public function transform($approval_settings)
    {
        $module_data = [];
        $approvar_data = [];
        $data = [];
        foreach ($approval_settings as $approval_setting)
        {
            ...
            {
                $business_member = $this->businessMember->where('id', $approvar->type_id)->get()->first();
                $member = $business_member ? $business_member->member : null;
                $profile = $member ? $this->profile->where('id', $member->profile_id)->get()->first() : null;
            ...
            $target_business_member = $approval_setting->target_type == Targets::EMPLOYEE ? $this->businessMember->where('id', $approval_setting->target_id)->get()->first() : null;
            $target_member = $target_business_member ? $target_business_member->member : null;
            $target_profile = $target_member ? $this->profile->where('id', $target_member->profile_id)->get()->first() : null ;
            $department = $approval_setting->target_type == Targets::DEPARTMENT ? $this->department->where('id', $approval_setting->target_id)->select('id','name')->get()->first() : null;
            $target_type = [
